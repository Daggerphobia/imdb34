<?php
include 'dbconn.php';
function remove_from_watchlist($request){
        ...
        while ($row = mysql_fetch_array($query, MYSQL_ASSOC)) {
          ...
          $query_watchlist_info = mysql_query("SELECT movie_id FROM watchlist WHERE user_id = $user_id AND movie_id = $movie_id ") or die(mysql_error()); 
           if(mysql_num_rows($query_watchlist_info)>0) {
              $query_watchlist = mysql_query("DELETE FROM watchlist WHERE user_id = $user_id AND movie_id = $movie_id") or die(mysql_error()); 
              if($query_watchlist){
                echo '{"result":1}';
              }else{
                echo '{"result":0}';
