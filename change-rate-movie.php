<?php
include 'dbconn.php';
function change_rate_movie($request){
        ...
        while ($row = mysql_fetch_array($query, MYSQL_ASSOC)) {
          ...
          $query_rated_info = mysql_query("SELECT rating FROM ratings WHERE user_id = $user_id AND movie_id = $movie_id LIMIT 1") or die(mysql_error()); 
           if(mysql_num_rows($query_rated_info)>0) {
              $query_rating = mysql_query("UPDATE  ratings SET rating = $rating WHERE user_id = $user_id AND movie_id = $movie_id ") or die(mysql_error()); 
              if($query_rating){
